...
clientRouter.post('/client', async (req, res) => {
    ...
    try {
        ...
        if (!liveTransfer) {
            ...
            if (leadError) {
                logger.error('Error fetching live transfer lead in endpoints/clients.js', {
                    route: '/client',
                    method: 'POST',
                    requesterId: req.agent?.id,
                    email: client.email,
        ...
        if (agentClientError) {
            logger.error('Error creating agent_clients record in endpoints/clients.js', {
                route: '/client',
                method: 'POST',
                requesterId: req.agent?.id,
...
clientRouter.patch('/clients', async (req, res) => {
    ...
    if (!clientId || !client) {
        ...
        return res.status(400).json({ error: 'Missing clientId or client payload' });
    }
