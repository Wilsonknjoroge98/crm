...
app.get('/clients', async (req, res) => {
  ...
  try {
    if (agentRole === 'agent') {
      ...
      const agencyAgentIds = agents.filter((a) => a.agency === agency).map((a) => a.uid);
...
app.get('/leads', async (req, res) => {
  ...
  try {
    if (agentRole === 'agent') {
      ...
      const agencyAgents = agents.filter((a) => a.agency === agency).map((a) => a.email);
  ...
  const patchAgentAccount = async () => {
    try {
      const response = await axios.request({
        ...
        data: { email: account.email, deliver: account.deliver, states: account.states },
        method: 'PATCH',
        // url: `${process.env.GSQ_BASE_URL}/customer-account`,
        url: `${process.env.GSQ_BASE_URL}/customer-account`,
      });
    ...
    const getHyrosSource = async () => {
      ...
      try {
        ...
        if (!source) {
          ...
          if (!source) {
            source = hyrosData.lastSource?.name || hyrosData.firstSource?.name || 'unknown';
          }
        }
...
app.post('/policy', async (req, res) => {
  ...
  const getLevel = async (agent, policy) => {
    ...
    if (!snapshot.empty) {
      console.log('Level found for agent:', agent.email, snapshot.docs[0].data());
  ...
  const calculateCommissions = async (agentData, agent, policy, premium) => {
    ...
    if (agent.uplineUid) {
      ...
      if (upline.uplineUid) {
        ...
        let secondUplineProductRate = productRates?.[String(secondUplineLevel)] / 100;
...
app.get('/premiums/per-lead', async (req, res) => {
  ...
  try {
    ...
    for (const id of agencyAgentIds) {
      ...
      const leads = await db.collection('leads').where('issuedTo', '==', agent.email).get();
    ...
    const perLeadDataArray = Object.entries(perLeadData).map(([name, data]) => ({
      name,
      ...data,
    }));
...
app.get('/premiums/monthly', async (req, res) => {
  ...
  try {
    ...
    const endDate = new Date(months[11].year, months[11].month + 1, 0, 23, 59, 59);
  ...
  try {
    ...
    for (const id of agencyAgentIds) {
      ...
      const activeOrPendingPolicies = policies.docs.filter((doc) => {
        const data = doc.data();
        return data.policyStatus === 'Active' || data.policyStatus === 'Pending';
      }).length;

      const persistencyRate = totalPolicies ? (activeOrPendingPolicies / totalPolicies) * 100 : 0;
    ...
    const persistencyDataArray = Object.entries(persistencyData).map(([agentName, data]) => ({
      name: agentName,
      ...data,
    }));
...
app.get('/close-rates', async (req, res) => {
  ...
  try {
    ...
    for (const id of agencyAgentIds) {
      ...
      const clients = await db.collection('clients').where('agentIds', 'array-contains', id).get();
    ...
    const closeRatesArray = Object.entries(closeRates).map(([agentName, data]) => ({
      name: agentName,
      ...data,
    }));
...
app.get('/insights', async (req, res) => {
  ...
  try {
    ...
    for (const ad of adsResp.data.data || []) {
      console.log('Fetched ad:', ad.name, ad.id);
  ...
  async function getSpendForAd(adId) {
    try {
      const insightsResp = await axios.get(`https://graph.facebook.com/v20.0/${adId}/insights`, {
        params: {
          fields: 'spend',
          time_range: JSON.stringify({ since, until }),
          access_token: accessToken,
      ...
      console.log(`Spend data for ad ${adId}:`, data);
      if (!data || data.length === 0) return 0;

      return parseFloat(data[0].spend || 0);
    } catch (error) {
      console.error(`Error fetching spend for ad ${adId}:`, error.response?.data || error);
      return 0;
    }
  }
  ...
  for (const [creative, sales] of Object.entries(
    ...
    const matched = policies.filter((p) => (p.source || 'unknown').trim() === creative);
    ...
    if (policy.splitPolicy && policy.agentIds.length === 2) {
      ...
      if (agent2.uplineUid) {
        ...
        if (upline && uplineProductRate) {
          console.log(`Found upline for agent 2: ${upline?.name} level: ${uplineLevel}`);
          const uplineCommission = Math.round(
...
//       const agent1 = agents.find((a) => a.uid === agentIds[0]);
//       const agent2 = agents.find((a) => a.uid === agentIds[1]);
...
//       const agent1Rate = getRate(productRates, agent1Level);
//       const agent2Rate = getRate(productRates, agent2Level);
...
//       const ref = db.ref(`policies/${policy.id}`);
//       ref.update({ productRate });
//     } else if (agentIds.length === 1) {
//       const carrierRates = PRODUCT_RATES[policy.carrier?.trim()];
//       const productRates = carrierRates?.[policy.policyType?.trim()];

//       const agent = agents.find((a) => a.uid === agentIds[0]);
//       const agentRate = getRate(productRates, agent.level);
